 ...
 */
interface UIBuilderDefaultNavBarProps<
  TRegistry extends ComponentRegistry = ComponentRegistry,
 ...
 */
interface UIBuilderCustomNavBarProps<
  TRegistry extends ComponentRegistry = ComponentRegistry,
> extends UIBuilderBaseProps<TRegistry> {
...
function MainLayout({ panelConfig }: { panelConfig: PanelConfig }) {
  ...
  return (
    ...
    >
      ...
      {/* Mobile Layout */}
      <div className="flex size-full flex-col overflow-hidden md:hidden">
        {selectedPanel?.content}
        <div className="absolute right-4 bottom-4 left-4 z-50">
          <div className="bg-primary flex justify-center rounded-full p-2 shadow-lg">
            {mainPanels.map((panel, index) => (
              <Button
                key={panel.title}
                variant={
...
export function PageConfigPanel({
  ...
  return (
    ...
    >
      ...
      {data && <TabsContent value="variables">{data.content}</TabsContent>}
    </Tabs>
  );
}
...
export function defaultConfigTabsContent() {
  return {
    ...
    appearance: {
      title: "Appearance",
      content: (
        <div className="flex flex-col gap-2 overflow-x-auto overflow-y-auto px-4 py-2">
          <ConfigPanel />
          <TailwindThemePanel />
        </div>
      ),
...
export function LoadingSkeleton() {
  return (
    <div
      ...
      className="bg-secondary/90 flex flex-1 flex-col gap-1"
    >
      <div className="bg-background h-16 w-full animate-pulse rounded-md"></div>
      <div className="flex flex-1 gap-1">
        <div className="bg-background w-1/4 animate-pulse rounded-md"></div>
        <div className="bg-muted-background/90 w-1/2 animate-pulse rounded-md"></div>
        <div className="bg-background w-1/4 animate-pulse rounded-md"></div>
      </div>
    </div>
  );
...
export const getDefaultPanelConfigValues = (
  ...
  return {
    ...
    pageConfigPanel: (
      <PageConfigPanel
        className="relative size-full overflow-y-auto pt-4 pb-20 md:pb-4"
        tabsContent={tabsContent}
      />
    ),
    editorPanel: <EditorPanel className="overflow-y-auto pb-20 md:pb-0" />,
    propsPanel: (
      <PropsPanel className="relative size-full overflow-y-auto px-4 pt-4 pb-20 md:pb-4" />
    ),
  };
};
