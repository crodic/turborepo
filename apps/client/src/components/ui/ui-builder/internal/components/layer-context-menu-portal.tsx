...
export const LayerContextMenuPortal: React.FC = () => {
  ...
  useEffect(() => {
    if (
      contextMenu.open &&
      contextMenuLayerIdRef.current &&
      selectedLayerId !== contextMenuLayerIdRef.current
    ) {
      closeContextMenu();
    }
  ...
  const menuContent = (
    <div
      ...
      className="bg-popover text-popover-foreground animate-in fade-in-0 zoom-in-95 z-[10000] min-w-[14rem] overflow-hidden rounded-md border p-1 shadow-md"
...
const MenuItem: React.FC<MenuItemProps> = ({
  ...
  return (
    <div
      ...
      className={cn(
        "hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm transition-colors outline-none select-none",
...
const MenuSeparator: React.FC = () => {
  return (
    <div className="bg-muted -mx-1 my-1 h-px" data-testid="menu-separator" />
  );
};

// Simple shortcut display component
const MenuShortcut: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  return (
    <span
