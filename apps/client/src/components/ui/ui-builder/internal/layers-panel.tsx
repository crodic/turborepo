...
export const LayersTree: React.FC<LayersTreeProps> = React.memo(
  ({
      ...
      (newLayers: unknown) => {
        if (Array.isArray(newLayers) && newLayers.length > 0) {
          ...
          const currentChildren =
            currentLayer && hasLayerChildren(currentLayer)
    ...
    const renderNode = useCallback(
      ...
      [handleNodeToggle, selectedLayerId, selectLayer, layers]
    );
    ...
    return (
      <DevProfiler id="LayersPanel" threshold={40}>
        <div
          ...
          className={cn(
            className,
            "flex size-full flex-col overflow-x-auto pl-4"
          )}
        >
          {layers.length > 0 ? (
            <>
              <DividerControl
                className="border-primary border-l border-dashed"
                addPosition={0}
                parentLayerId={selectedPageId}
              />
              {renderTree()}
              <div className="relative">
                <div className="border-primary bg-background absolute bottom-0 left-0 h-4 w-[1px] border-l border-dashed" />
              </div>
              <DividerControl
                className="border-primary border-l border-dashed"
                parentLayerId={selectedPageId}
