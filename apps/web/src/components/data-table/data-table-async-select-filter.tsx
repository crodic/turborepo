...
interface DataTableAsyncSelectContentProps<TData, TValue> {
  column?: Column<TData, TValue>
  title?: string
  multiple?: boolean
  limit?: number
...
function DataTableAsyncSelectContent<TData, TValue>({
  column,
  title,
  multiple,
  fetchOptions,
  ...
  onClose,
  onReset,
}: DataTableAsyncSelectContentProps<TData, TValue>) {
  ...
  const searchKey = column?.columnDef.meta?.searchKey ?? 'search'
  ...
  const rowVirtualizer = useVirtualizer({
    ...
    estimateSize: () => 36,
    overscan: 8,
    initialRect: { width: 256, height: 288 },
  })
  ...
  const onItemSelect = React.useCallback(
    ...
    [column, multiple, selectedValues, onClose]
  )
  ...
  return (
    <Command shouldFilter={false}>
      ...
      <CommandList ref={setScrollElement} className='max-h-72 overflow-y-auto'>
        <CommandEmpty>
