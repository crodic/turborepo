  ...
  return (
    <MediaPlayerContext.Provider value={contextValue}>
      <RootPrimitive
        ...
        onKeyUp={onKeyUp}
        className={cn(
          'dark bg-background focus-visible:border-ring focus-visible:ring-ring/50 relative isolate flex size-full flex-col overflow-hidden rounded-lg outline-none focus-visible:ring-[3px] data-disabled:pointer-events-none data-disabled:opacity-50 [&_video]:relative [&_video]:object-contain',
...
function MediaPlayerLoading(props: MediaPlayerLoadingProps) {
  ...
  return (
    <LoadingPrimitive
      ...
      {...loadingProps}
      className={cn(
        'animate-in fade-in-0 zoom-in-95 pointer-events-none absolute inset-0 z-50 flex items-center justify-center duration-200',
        className
      )}
    >
      {children ?? (
...
function MediaPlayerVolumeIndicator(props: DivProps) {
  ...
  return (
    ...
    >
      <div className='animate-in fade-in-0 zoom-in-95 flex flex-col items-center gap-3 rounded-lg bg-black/30 px-6 py-4 text-white backdrop-blur-xs duration-200'>
        <div className='flex items-center gap-2'>
...
function MediaPlayerSeekForward(props: MediaPlayerSeekForwardProps) {
  ...
  const [, rawSeekableEnd = 0] = useMediaSelector(
    (state) => state.mediaSeekable ?? [0, 0]
  )
  const seekableEnd = rawSeekableEnd > 0 ? rawSeekableEnd : mediaDuration
  const isDisabled = disabled || context.disabled
...
function MediaPlayerSeek(props: MediaPlayerSeekProps) {
  ...
  const mediaDuration = useMediaSelector((state) => state.mediaDuration ?? 0)
  const [seekableStart = 0, rawSeekableEnd = 0] = useMediaSelector(
    (state) => state.mediaSeekable ?? [0, 0]
  )
  const seekableEnd = rawSeekableEnd > 0 ? rawSeekableEnd : mediaDuration
