...
const DROP_ZONE_DISABLED_BG =
  'bg-gray-200/20 border border-gray-300 border-dashed rounded-md'
...
export const DropZone: React.FC<DropZoneProps> = ({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn('relative', !isDropValid && 'opacity-60', className)}
    ...
    >
      ...
      {isActive && !children && (
        <div className={cn('h-8 transition-all duration-200', getDropZoneBg())}>
          <div
            className={cn(
  ...
  React.useLayoutEffect(() => {
    ...
    if (nextSibling) {
      ...
      if (nextSibling.hasAttribute('data-layer-id')) {
        ...
        siblingElement = nextSibling.querySelector(
          '[data-layer-id]'
        ) as HTMLElement
      }
    }
    ...
    if (siblingElement) {
      ...
      const directChildLayers = Array.from(childLayers).filter((el) => {
        ...
        return (
          closestLayerParent === parentContainer || closestLayerParent === null
        )
      })
      const lastChild = directChildLayers[
        directChildLayers.length - 1
      ] as HTMLElement
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'pointer-events-auto absolute z-[999]',
        "before:absolute before:content-['']",
        BEFORE_TRANSITION,
        'before:pointer-events-none',
        // Disabled state visual cue
        !isDropValid && 'cursor-not-allowed opacity-60',
        ...
        !useCalculatedPositioning &&
          !style && [
            ...
            layoutType === 'flex-row' && [
              ...
              getIndicatorStyles(
                `${DROP_INDICATOR_HOVER} ${BEFORE_ROUNDED_HOVER} before:h-16 before:w-2`,
                `${DROP_INDICATOR_DEFAULT} ${BEFORE_ROUNDED_DEFAULT} before:h-6 before:w-1`
              ),
            ],
            ...
            layoutType === 'inline' && [
              'top-0 bottom-0 -left-1 w-2',
              BEFORE_CENTER_TRANSFORM,
              'before:h-4 before:w-0.5',
              getIndicatorStyles(
                `${DROP_INDICATOR_INLINE_HOVER} ${BEFORE_ROUNDED_HOVER} before:h-8 before:w-1`,
                `${DROP_INDICATOR_INLINE_DEFAULT} ${BEFORE_ROUNDED_DEFAULT} before:h-3 before:w-0.5`
              ),
            ],
            // Vertical layout (flex-col, block): horizontal lines on top edge
            (layoutType === 'flex-col' || layoutType === 'block') && [
              '-top-1 right-0 left-0 h-2',
              BEFORE_CENTER_TRANSFORM,
        ...
        style && [
          isDropValid
            ? [
                'before:rounded-sm before:bg-blue-500/60',
                isOver && 'before:bg-blue-600 before:shadow-md',
              ]
            : [
                'before:rounded-sm before:bg-gray-400/30',
                isOver && 'before:bg-gray-500/40',
              ],
        ]
      )}
