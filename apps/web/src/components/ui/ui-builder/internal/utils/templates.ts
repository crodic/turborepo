...
const generateVariableIdentifiers = (
  variables: Variable[]
): Map<string, string> => {
...
const generateFunctionIdentifiers = (
...
export const pageLayerToCode = (
  ...
  const functionIdentifiers = generateFunctionIdentifiers(
    functionMetadataIds,
    functionVarReservedIds
  )
  ...
  const pageProps = generatePropsString(
    restOfProps,
    variables,
    variableIdentifiers,
    functionIdentifiers
    ...
    if (componentDefinition && componentDefinition.from) {
      imports.add(
        createFromString(
          ...
          layer.type,
          componentDefinition.isFromDefaultExport
        )
      )
      if (hasLayerChildren(layer)) {
...
export const generatePropsString = (
  ...
  functionMetadata.forEach((funcId, propName) => {
    const sanitizedId =
      functionIdentifiers?.get(funcId) ?? toValidIdentifier(funcId)
...
const generateVariablePropsInterface = (
  ...
  const hasFunctionMetadata =
    functionMetadataIds && functionMetadataIds.size > 0
  ...
  const variableTypes = regularVariables
    .map((variable) => {
      let tsType = 'unknown'
  ...
  const functionVariableTypes = functionVariables.map((variable) => {
    ...
    const funcDef =
      typeof funcId === 'string' ? functionRegistry?.[funcId] : undefined
