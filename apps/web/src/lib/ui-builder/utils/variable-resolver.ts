...
export function resolveVariableReferences(
  ...
  for (const [propName, functionId] of Object.entries(functionMetadata)) {
    if (functionRegistry) {
      ...
      if (funcDef) {
        ...
        console.warn(
          `Function "${functionId}" not found in function registry for prop "${propName}"`
        )
  ...
  for (const [key, value] of Object.entries(props)) {
    ...
    if (isVariableReference(value)) {
        ...
        if (variable.type === 'function') {
          ...
          if (functionRegistry) {
            ...
            if (funcDef) {
              ...
              console.warn(
                `Function "${varFunctionId}" not found in function registry`
              )
      ...
      resolved[key] = resolveVariableReferences(
